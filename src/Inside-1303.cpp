...
void keyboard(){
    ...
    if (!over && !stop && !starting){
        ...
        if (GetAsyncKeyState('1') & 0x8000) exit(0);
            ...
            debugs = string("Language ")+lang["Name"];
            if (lang["Name"][0] == 'E') lang = chinesePack;
            else if (lang["Name"][0] == 'C') lang = englishPack;
            else lang = englishPack;
            stop=false;
        }
...
void draw(){
    ...
    if (starting) printf("%s: %s\n",lang["hud.mode.text"],lang["hud.mode.select"]);
    else printf("%s: %s vs %s\n",lang["hud.mode.text"],(aiControl1)?"AI":"Player",(aiControl)?"AI":"Player");
    ...
    if (over) {
        ...
        printf("%s!",lang["over.title"]);
        setConsolePos(45,11);
        printf("%s: Player%s",lang["over.winner"],(player.getHealth() <= 0)?"2":"1");
        setConsolePos(47,13);
...
void init(){
    ...
    stop=false;
    starting = true;
    SetConsoleTitle("Friend War Inside-1303");
    SetConsoleTextAttribute(STDOUT_HANDLE,0x07);
    printf(" ----------------------------------\n");Sleep(11);
    printf(" |                                |\n");Sleep(10);
    printf(" |                                |\n");Sleep(10);
    ...
    printf(" |                                |\n");Sleep(10);
    printf(" ----------------------------------\n");Sleep(11);
    printf(" Version Inside-1303    Made in Tianxun\n\n\n");Sleep(10);
    printf(" Loading: ");
    SetConsoleTextAttribute(STDOUT_HANDLE,0x88);
    printf("                                                  \n");Sleep(10);
    SetConsoleTextAttribute(STDOUT_HANDLE,0x07);
