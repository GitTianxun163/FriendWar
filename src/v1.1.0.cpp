...
Update Log:
    1. 加入边框显示
    2. 加入AI功能（测试中，按ESC呼出游戏菜单，按3(不能是小键盘上的)，这样玩家B就会成为AI操控(反应太快了，我都打不赢，以后会优化)）
...
void draw(){
    ...
    if (stop) {
        ...
        setConsolePos(47,19);
        printf("3 : Open/Close AI for player2(Testing)");
    }
}
...
void init(){
    over=false;
    stop=false;
    SetConsoleTitle("Friend War v1.1.0");
    SetConsoleTextAttribute(STDOUT_HANDLE,0x07);
    printf(" ----------------------------------\n");Sleep(11);
    printf(" |                                |\n");Sleep(10);
    printf(" |                                |\n");Sleep(10);
    ...
    printf(" |                                |\n");Sleep(10);
    printf(" ----------------------------------\n");Sleep(11);
    printf(" Version v1.1.0    Made in Tianxun\n\n\n");Sleep(10);
    printf(" Loading: ");
    SetConsoleTextAttribute(STDOUT_HANDLE,0x88);
    printf("                                                  \n");Sleep(10);
    SetConsoleTextAttribute(STDOUT_HANDLE,0x07);
...
void SetConsoleWindowSize(short width,short height){
    ...
    SetConsoleScreenBufferSize(STDOUT_HANDLE, coord ); // 设置缓冲尺寸
}
...
void updateAi() {
    ...
    */
    if (player2.x < player.x) {
        player2.forword();
        if (player2.CD>=MAX_CD && player2.bullet_num>0 && player2.y == player.y) {
            bulletList.push_back(Bullet(&player2));
            player2.setNewB(false);
            player2.bullet_num--;
            player2.CD=0;
    ...
    if (player2.bullet_num < 5) {
        player2.setNewB(true);
    }
}
